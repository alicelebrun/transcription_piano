...
int initialiser_graphique()
{
  ...
  if (IMG_Init(IMG_INIT_PNG) == 0) {
    SDL_Log("Erreur lors de l'initialisation de la gestion des images: %s\n", SDL_GetError());
    return 1;
  }
  return 0;
}

/* Charge le fichier png dans une surface SDL2 */
...
{
  SDL_Surface *surface = IMG_Load(nom_image);
  if (surface == NULL) {
    SDL_Log("Erreur lors du chargement de l'image %s: %s\n", nom_image, IMG_GetError());
    return 1;
  }
