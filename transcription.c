...
int calculer_spectrogramme(double * donnees_son, double * instant_son, int debut, int fin, double ** spectrogramme, double ** frequences) { //int début et int fin sont des indices qui permettront de découper le signal en bloc ne contenant qu'une note chacun. *spectrogramme est le spectogramme qui sera rempli dans la fonction (c'est un tableau qui contiendra l'amplitude de la fft), et frequences est le tableau contenant les fréquences associées au spectrogramme.
	...
	int demiBloc = bloc/2;
	for(int i = 0; i<demiBloc; ++1) { //fréquence symétrique donc on stocke une seule fois l'info
	...
	fftw_plan plan = fftw_plan_dft_1d(bloc, signal_complexe, fft, FFTW_FORWARD, FFT_ESTIMATE); //prepare le calucul de la trans de fourier, bloc = taille du signal qu'on manipule,  fft = pointeur vers le résultat de la transformée de Fourier, FFT_FORWARD signifie qu'on passe du domaine temporel au domaine fréquentiel (et pas l'inverse), FFT_ESTIMATE = moyen efficace de trouver les paramètres de l'algoritme de FFT.
	fftw_execute(plan);//Calcul effecti de la FFT
	for(int i = 0; i< demiBloc; ++i) {//on part de 0 jusu'à demiBloc-1, on a donc bien demiBloc indices à balayer -> fréquence symétrique donc on stocke une seule fois l'info
